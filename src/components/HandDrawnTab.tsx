...
import { CreativeFactors, PipelineData, PipelineStatus } from "../types";
...
interface HandDrawnTabProps {
  ...
  onExport: (tab: "lyrics" | "copywriting", title: string, creativeText: string) => void;
}
...
export default function HandDrawnTab({
  ...
  const runPipeline = async () => {
    ...
    try {
      ...
      if (!promptRes.ok) {
        throw new Error("智能搭配提示词失败，请稍后重试。");
      }
      ...
      const themeName = activePreset ? PRESETS[activePreset as keyof typeof PRESETS].name : "custom";
      ...
      if (!imageRes.ok) {
        throw new Error("自动进行AI艺术绘图失败，请稍后重试。");
      }
      ...
      // Step 3: Deconstruct visual & Auto image recognition
      setStatus("deconstructing_visual");
      const deconstructRes = await fetch("/api/deconstruct-visual", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt: promptData.englishPrompt,
      ...
      if (!deconstructRes.ok) {
        throw new Error("自动识图失败，请稍后重试。");
      }
      const deconstructData = await deconstructRes.json();
      
      setPipeline((prev) => ({
        ...prev,
        deconstructed: deconstructData
      }));
  ...
  const triggerExport = (tab: "lyrics" | "copywriting") => {
    ...
    onExport(tab, pipeline.promptResult.title, summary);
  };
    ...
    <div className="flex w-full min-h-[600px] flex-col md:flex-row" id="hand-drawn-container">
      ...
      <section className="w-full md:w-[55%] p-6 md:p-12 border-b md:border-b-0 md:border-r border-gray-100 flex flex-col justify-between" id="creative-matrix-section">
        <div>
          ...
          {/* Form Inputs with exact Icons */}
          <div className="space-y-4 mb-10" id="matrix-inputs-container">
            {/* Subject */}
            ...
            {/* Scene */}
            <div className="relative" id="input-scene-group">
              <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none text-gray-400">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 bg-gray-50 border-0 rounded-xl focus:ring-2 focus:ring-black/10 focus:bg-white text-sm text-gray-800 placeholder-gray-400 outline-none transition-all"
                placeholder="场景背景 (例如：落日余晖下金黄的麦田，旁边放着原木酿酒桶)"
                id="input-scene"
              />
            </div>

            {/* Lighting */}
            <div className="relative" id="input-lighting-group">
              <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none text-gray-400">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 bg-gray-50 border-0 rounded-xl focus:ring-2 focus:ring-black/10 focus:bg-white text-sm text-gray-800 placeholder-gray-400 outline-none transition-all"
                placeholder="光影氛围 (例如：柔和逆光，夕阳穿透酒液呈剔透琥珀色，微泛金色光斑)"
                id="input-lighting"
              />
            </div>

            {/* Style */}
            <div className="relative" id="input-style-group">
              <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none text-gray-400">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 bg-gray-50 border-0 rounded-xl focus:ring-2 focus:ring-black/10 focus:bg-white text-sm text-gray-800 placeholder-gray-400 outline-none transition-all"
                placeholder="视觉风格 (例如：高端食品广告摄影，微距景深，温润高对比大片质感)"
                id="input-style"
              />
            </div>

            {/* Posture */}
            <div className="relative" id="input-posture-group">
              <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none text-gray-400">
                <Activity className="w-5 h-5" />
              </div>
              <input
                type="text"
                value={factors.posture}
                onChange={(e) => handleInputChange("posture", e.target.value)}
                className="w-full pl-12 pr-4 py-4 bg-gray-50 border-0 rounded-xl focus:ring-2 focus:ring-black/10 focus:bg-white text-sm text-gray-800 placeholder-gray-400 outline-none transition-all"
                placeholder="动态细节 (例如：泡沫挂壁，一滴冷凝水正顺着剔透杯壁缓缓滑落)"
                id="input-posture"
              />
            </div>
        ...
        <div id="cta-button-container">
          ...
          <button
            ...
            id="run-pipeline-btn"
            className={`w-full text-white py-5 rounded-[2rem] flex items-center justify-center space-x-3 text-lg font-bold transition-all transform active:scale-95 shadow-lg select-none cursor-pointer ${
      ...
      <section className="w-full md:w-[45%] p-6 md:p-12 flex flex-col justify-between" id="pipeline-monitor-section">
        <div className="w-full">
          <h2 className="text-xl md:text-2xl font-bold mb-8 text-gray-900">流水线监视器</h2>

          {/* Workflow progress line visual */}
          <div className="space-y-6 md:space-y-8 mb-8 relative pl-3" id="steps-progress-bar">
        ...
        <div className="w-full flex-grow flex flex-col justify-center" id="monitor-content-area">
            ...
            {status === "idle" && (
              ...
              >
                ...
                <p className="text-xs md:text-sm leading-relaxed text-gray-500">
                  选择上方预设或输入创意因子，点击【一键全链条智绘】，系统将自动生成提示词、绘制图片并识别。
                </p>
              </motion.div>
            )}
            ...
            {status === "completed" && pipeline.promptResult && pipeline.imageUrl && (
              ...
              >
                ...
                <div className="flex space-x-2.5 pt-2" id="cross-export-actions">
                  ...
                  <button
                    onClick={() => triggerExport("copywriting")}
                    className="flex-1 bg-gray-50 border border-gray-200 hover:bg-gray-100 text-gray-700 font-bold py-3 px-4 rounded-xl text-xs flex items-center justify-center space-x-1.5 transition-all cursor-pointer"
                  >
                    <span>爆款文案</span>
                    <ArrowRight className="w-3.5 h-3.5" />
